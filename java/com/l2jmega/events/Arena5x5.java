...
public class Arena5x5 implements Runnable
{
	...
	public boolean register(Player player, Player assist, Player assist2, Player assist3, Player assist4)
	{
		if (SiegeParticipationUtil.isPlayerOrClanInActiveSiege(player)
			|| SiegeParticipationUtil.isPlayerOrClanInActiveSiege(assist)
			|| SiegeParticipationUtil.isPlayerOrClanInActiveSiege(assist2)
			|| SiegeParticipationUtil.isPlayerOrClanInActiveSiege(assist3)
			|| SiegeParticipationUtil.isPlayerOrClanInActiveSiege(assist4))
		{
		...
		final Player[] partyMembers = new Player[] { player, assist, assist2, assist3, assist4 };
		for (Player member : partyMembers)
		{
			if (member != null && OlympiadManager.getInstance().isParallelEventBlockedFor(member))
			{
				if (player != null && player.isOnline())
					player.sendMessage("Tournament: A party member cannot register while registered for or fighting in Olympiad.");
				member.sendMessage("Tournament: You cannot register while registered for or fighting in Olympiad.");
				return false;
			}
		}
		...
		for (Player member : partyMembers)
		{
			if (member != null && RandomFight.isCommittedToRandomFight(member))
			{
				if (player != null && player.isOnline())
					player.sendMessage("Tournament: A party member is registered or fighting in Random Fight.");
				member.sendMessage("Tournament: Leave Random Fight before joining the tournament.");
				return false;
			}
		}

		if ((Config.bs_COUNT_5X5 == 0) && (AbstractArenaPair.isForbiddenSupportClass(player) || AbstractArenaPair.isForbiddenSupportClass(assist) || AbstractArenaPair.isForbiddenSupportClass(assist2) || AbstractArenaPair.isForbiddenSupportClass(assist3) || AbstractArenaPair.isForbiddenSupportClass(assist4)))
		{
			if (player != null && player.isOnline())
				player.sendMessage("Tournament: Support classes are disabled for 5x5.");
			return false;
		}
		
		if (Arena1x1.getInstance().isRegistered(player) || Arena1x1.getInstance().isRegistered(assist) || Arena1x1.getInstance().isRegistered(assist2) || Arena1x1.getInstance().isRegistered(assist3) || Arena1x1.getInstance().isRegistered(assist4)
			|| Arena2x2.getInstance().isRegistered(player) || Arena2x2.getInstance().isRegistered(assist) || Arena2x2.getInstance().isRegistered(assist2) || Arena2x2.getInstance().isRegistered(assist3) || Arena2x2.getInstance().isRegistered(assist4))
		{
			if (player != null && player.isOnline())
				player.sendMessage("Tournament: One of your party members is already registered in another mode.");
			return false;
		}
	...
	private void restoreOpponents(List<Pair> opponents)
	{
		synchronized (registrationLock)
		{
			for (Pair p : opponents)
			{
				...
				boolean exists = isRegistered(p.getLeader()) || isRegistered(p.getAssist()) || isRegistered(p.getAssist2()) || isRegistered(p.getAssist3()) || isRegistered(p.getAssist4());
	...
	private class Pair extends AbstractArenaPair
	{
		...
		{
			super(leader, assist, assist2, assist3, assist4);
			this.leader = leader;
			this.assist = assist;
	...
	private class EvtArenaTask implements Runnable
	{
		...
		private void finishDuel()
		{
			...
			arena.setFree(true);
			Player p1a = pairOne.getLeader(), p1b = pairOne.getAssist(), p1c = pairOne.getAssist2(), p1d = pairOne.getAssist3(), p1e = pairOne.getAssist4();
			if (ArenaTask.is_started() && p1a instanceof FakePlayer && p1b instanceof FakePlayer && p1c instanceof FakePlayer && p1d instanceof FakePlayer && p1e instanceof FakePlayer)
				TournamentAI.scheduleFakeReRegister5x5((FakePlayer) p1a, (FakePlayer) p1b, (FakePlayer) p1c, (FakePlayer) p1d, (FakePlayer) p1e);
			Player p2a = pairTwo.getLeader(), p2b = pairTwo.getAssist(), p2c = pairTwo.getAssist2(), p2d = pairTwo.getAssist3(), p2e = pairTwo.getAssist4();
			if (ArenaTask.is_started() && p2a instanceof FakePlayer && p2b instanceof FakePlayer && p2c instanceof FakePlayer && p2d instanceof FakePlayer && p2e instanceof FakePlayer)
				TournamentAI.scheduleFakeReRegister5x5((FakePlayer) p2a, (FakePlayer) p2b, (FakePlayer) p2c, (FakePlayer) p2d, (FakePlayer) p2e);
		}
		...
		private boolean portPairsToArena()
		{
			...
			for (Arena arena : arenas)
			{
				if (arena.isFree)
				{
					this.arena = arena;
					arena.setFree(false);
					for (Player p : new Player[] { pairOne.getLeader(), pairOne.getAssist(), pairOne.getAssist2(), pairOne.getAssist3(), pairOne.getAssist4(), pairTwo.getLeader(), pairTwo.getAssist(), pairTwo.getAssist2(), pairTwo.getAssist3(), pairTwo.getAssist4() })
						if (p != null && p.isOnline() && p.isInParty())
							p.leaveParty();
					pairOne.removePet();
					pairTwo.removePet();
