...
public class TvT implements EventTask
{
	...
	public static boolean startJoin()
	{
		...
		Announcement.toAllOnlinePlayers(new ExShowScreenMessage(Config.NAME_TVT + " registration started in " + _joiningLocationName + "!", 6000));
		...
		ThreadPool.schedule(() -> {
			if (_joining && !_teleport && !_started)
				Announcement.toAllOnlinePlayers(new ExShowScreenMessage(Config.NAME_TVT + " registration is open in " + _joiningLocationName + ".", 5000));
		}, 5000L);
		
		if (FakePlayerConfig.ALLOW_FAKE_PLAYER_TVT && FakePlayerConfig.isAutomatedFakePopulationEnabled())
		{
	...
	public static boolean startEvent()
	{
		...
		for (Player player : _players)
		{
			if (player != null && player.isOnline())
			{
				...
				{
					((FakePlayer) player).setFakeEvent(true);
				}
			}
		}
					...
					for (Player player : _players)
					{
						if (player != null)
						{
							if (player.isOnline())
							{
								...
								if (player instanceof FakePlayer)
								{
									((FakePlayer) player).despawnPlayer();
								}
							}
							else
							{
	...
	{
		if (SiegeParticipationUtil.isPlayerOrClanInActiveSiege(eventPlayer))
		{
			eventPlayer.sendMessage("You cannot register for TvT while your clan is participating in an active siege.");
			return false;
		}
		...
		if (!Config.Allow_Same_HWID_On_tvt && !eventPlayer.isGM() && !eventPlayer.isPhantom())
		{
			...
			if (hwid != null && _playersShuffle != null && !_playersShuffle.isEmpty())
			{
				for (Player p : _playersShuffle)
				{
					String phwid = (p != null) ? p.getHWID() : null;
		...
		else if (RandomFight.isCommittedToRandomFight(eventPlayer))
		{
			eventPlayer.sendMessage("You are registered or fighting in Random Fight. Leave it first.");
			return false;
		}
		else if (OlympiadManager.getInstance().isParallelEventBlockedFor(eventPlayer))
		{
	...
	public static void addPlayer(Player player, String teamName)
	{
		...
		if (player instanceof FakePlayer fp && !(fp.getFakeAi() instanceof CombatAI))
			fp.assignDefaultAI();
	}
	...
	public static void removePlayer(Player player)
	{
		...
		{
			final String teamName = player._teamNameTvT;
			if (!_joining)
			{
			...
			if (player instanceof FakePlayer)
				((FakePlayer) player).setFakeEvent(false);
