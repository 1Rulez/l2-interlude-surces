...
public class RaidZoneManager
{
    ...
    private static boolean checkPlayersKickTask(Player activeChar, Integer numberBox)
    {
        ...
        // Use optimized zone player lookup instead of scanning all players
        // Count players with same HWID in raid zones
        int sameHWIDCount = ZonePlayerManager.getInstance().countHWIDInZones(activeChar.getHWID(), ZoneId.RAID, ZoneId.RAID_NO_FLAG);
        
        return sameHWIDCount > numberBox;
    }
    ...
    private static boolean checkPlayersKickTask_ip(Player activeChar, Integer numberBox, Collection<Player> world)
    {
        ...
        // Use optimized zone player lookup instead of scanning all players
        Set<Player> raidPlayers = ZonePlayerManager.getInstance().getPlayersInZones(ZoneId.RAID, ZoneId.RAID_NO_FLAG);
        
        String activeCharIP = activeChar.getClient().getConnection().getInetAddress().getHostAddress();
        int sameIPCount = 0;
        ...
        for (Player player : raidPlayers)
        {
            ...
            if (activeCharIP.equals(playerIP) && IPProtection(playerIP))
            {
                sameIPCount++;
                if (sameIPCount >= numberBox)
                    return true;
            }
        }
        return false;
