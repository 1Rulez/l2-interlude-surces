...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	private static final String[] ADMIN_COMMANDS =
	{
		...
		"admin_setrace", // changes char's race
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.startsWith("admin_setclass"))
		{
			try
			{
				...
				if (valid && (player.getClassId().getId() != classidval))
				{
					...
					player.setCharacterRace(player.getTemplate().getRace().ordinal());
					...
					// Hard client refresh without delayed respawn.
					player.decayMe();
					player.spawnMe(x, y, z);
					player.sendPacket(new UserInfo(player));
					player.broadcastUserInfo();
					
					final String newClassName = player.getTemplate().getClassName();
					if (player != activeChar)
						player.sendMessage("A GM changed your class to " + newClassName + ".");
					
					activeChar.sendMessage(player.getName() + " is now a " + newClassName + ".");
				}
				else
		...
		else if (command.startsWith("admin_setrace"))
		{
			...
			try
			{
				...
				if (!st.hasMoreTokens())
				{
					activeChar.sendMessage("Usage: //setrace <0-4 | HUMAN | ELF | DARK_ELF | ORC | DWARF>");
					return false;
				}
				
				final ClassRace newRace = parseRace(st.nextToken());
				player.setCharacterRace(newRace.ordinal());
				player.store();
				...
				if (player != activeChar)
					player.sendMessage("A GM corrected your race to " + newRace + ".");
				
				activeChar.sendMessage(player.getName() + " race set to " + newRace + ".");
			}
			catch (Exception e)
			{
				activeChar.sendMessage("Usage: //setrace <0-4 | HUMAN | ELF | DARK_ELF | ORC | DWARF>");
	...
	private static void gatherCharacterInfo(Player activeChar, Player player, String filename)
	{
		...
		if (client != null)
		{
			...
			if (client.getConnection() != null && client.getConnection().getInetAddress() != null)
	...
	private static ClassRace parseRace(String value)
	{
		try
		{
			final int ordinal = Integer.parseInt(value);
			if (ordinal >= 0 && ordinal < ClassRace.values().length)
				return ClassRace.values()[ordinal];
		}
		catch (NumberFormatException e)
		{
		}
		
		return ClassRace.valueOf(value.toUpperCase());
	}
