...
public class AdminCustom implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.equals("admin_partyzone"))
		{
			...
			|| command.equals("admin_spawn_phantomtvt")
			|| command.equals("admin_spawn_phantomfarm")
			|| command.equals("admin_spawn_phantom")
			|| command.equals("admin_spawn_archer"))
		{
			AdminAdmin.showMainPage(activeChar, "main_menu.htm");
			ThreadPool.schedule(new PhantomReloadTask(activeChar), 1);
		}
		return true;
	}
	...
	{
		ThreadPool.schedule(new InitEventArenaTask(), 1);
	}
	
	private static void finishEventArena()
	{
		ThreadPool.schedule(new FinishEventArenaTask(), 1);
	}
	
	private static class InitEventArenaTask implements Runnable
	{
		@Override
		public void run()
		{
	...
	private static class FinishEventArenaTask implements Runnable
	{
		@Override
		public void run()
		{
	...
	{
		ThreadPool.schedule(new InitPartyZoneTask(), 1);
	}
	
	private static void finishPartyZone()
	{
		ThreadPool.schedule(new FinishPartyZoneTask(), 1);
	}
	
	private static class InitPartyZoneTask implements Runnable
	{
		@Override
		public void run()
		{
	...
	private static class FinishPartyZoneTask implements Runnable
	{
		@Override
		public void run()
		{
	...
	public static void VoteZone() {
		ThreadPool.schedule(new VoteZoneTask(), 1L);
	}

	private static class VoteZoneTask implements Runnable
	{
		@Override
		public void run()
		{
			...
			_log.info("[VoteZone]: Started!");
		}
	}
	
	private static class PhantomReloadTask implements Runnable
	{
		private final Player _admin;

		PhantomReloadTask(Player admin)
		{
			_admin = admin;
		}
		...
		public void run()
		{
			...
			_log.info("[Fake Players]: Restarted!");
			if (_admin != null && _admin.isOnline())
				_admin.sendMessage("SYS: Fake Players system reloaded.");
		}
	}
