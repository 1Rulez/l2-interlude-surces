...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.equals("admin_reward_all"))
		{
			try
			{
				...
				activeChar.sendMessage(players.size() + " players rewarded with " + getObjectName(id) + ".");
			}
			catch (Exception e)
			{
	...
	private static void createItem(Player activeChar, Player target, int id, int num, int radius, boolean sendGmMessage)
	{
		...
		if (template == null)
		{
			final DressMe skin = DressMeData.getInstance().getItemId(id);
			if (skin == null)
			{
				activeChar.sendMessage("This item or skin doesn't exist.");
				return;
			}
			
			applySkin(activeChar, target, skin, radius, sendGmMessage);
			return;
		}
	...
	private static void applySkin(Player activeChar, Player target, DressMe skin, int radius, boolean sendGmMessage)
	{
		if (radius > 0)
		{
			final List<Player> players = activeChar.getKnownTypeInRadius(Player.class, radius);
			...
			if (sendGmMessage)
				activeChar.sendMessage(players.size() + " players received skin ID " + skin.getItemId() + " in a " + radius + " radius.");
		}
		else
		{
			target.setDress(skin);
			target.broadcastUserInfo();
			
			if (sendGmMessage)
				activeChar.sendMessage("You have applied skin ID " + skin.getItemId() + " to " + target.getName() + ".");
		}
	}
