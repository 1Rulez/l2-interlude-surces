	...
	public boolean useVoicedCommand(String command, Player activeChar, String target) {
		...
		if ((command.equals("pvpEvent") || command.equals("pvpevent")) && Config.PVP_EVENT_ENABLED){
			...
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				PreparedStatement statement = con.prepareStatement("SELECT char_name,pvpkills,online FROM characters WHERE accesslevel=0 ORDER BY pvpkills DESC, char_name ASC LIMIT 15");
			...
			tb.append("</tr>");
			try (Connection con = L2DatabaseFactory.getInstance().getConnection()) {
				PreparedStatement statement = con.prepareStatement("SELECT char_name,pkkills,online FROM characters WHERE accesslevel=0 ORDER BY pkkills DESC, char_name ASC LIMIT 15");
				ResultSet result = statement.executeQuery();
				int pos = 0;
				while (result.next()) {
