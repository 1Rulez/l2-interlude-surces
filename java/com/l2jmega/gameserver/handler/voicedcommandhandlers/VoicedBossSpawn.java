...
public boolean useVoicedCommand(String command, Player activeChar, String target) 
  {
	...
	else if (command.startsWith("checkZone"))
	{
		try
		{
			// Args come in `target` (see RequestBypassToServer: voiced_checkZone <id> <type>).
			final String argLine = (target != null && !target.trim().isEmpty()) ? target.trim() : "";
			StringTokenizer st = new StringTokenizer(argLine, " ");
			if (st.countTokens() < 2)
			{
				activeChar.sendMessage("Invalid boss check request.");
				return true;
			}
		...
		try
		{
			...
			for (Player player : insideRadius)
			{
				if (player.getClan() != null)
				{
					...
					{
						cInsideRadius.put(player.getClan().getAllyName(), new ArrayList<>());
					...
					{
						cInsideRadius.put(player.getClan().getName(), new ArrayList<>());
