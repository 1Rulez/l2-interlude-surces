...
import com.l2jmega.gameserver.taskmanager.AutoGoldBar;
...
public class VoicedMenu implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		...
		else if (command.equals("mission"))
		{
			...
			if(activeChar.isVip()){
				final Integer skinId = parseSkinId(command, target);
				if (skinId == null) {
	...
	{		
		...
		{
		    html.replace("%anonymouspvp%",  AnonymousPvPEvent.formatMenuTime(AnonymousPvPEvent.scheduleNextEventTime()
		        )
		    );
		}
		...
		{
		    html.replace("%raceofwar%", RacesOnWar.getMenuTime());
		}
		else
		{
		    html.replace("%raceofwar%", "00:00");
		}
		
		if(Config.BOSS_EVENT_ENABLED)
		{
		    html.replace("%bossevent%", BossEvent.getInstance().getMenuTime());
		}
		
		if(Config.ALLOW_RANDOM_FIGHT)
		{
		    html.replace("%randomfight%", RandomFight.getMenuTime());
		}

		
		if(L2ElysianUltimateZone.EVENT_ACTIVE)
		{
		    html.replace("%ultimate%", L2ElysianUltimateZone.getMenuTime());
		}
		else
		{
		    html.replace("%ultimate%", "00:00");
		}
