...
public class VoicedMission implements IVoicedCommandHandler {
	...
	private static final String[] VOICED_COMMANDS = new String[] { "select_m", "voiced_select_m" };
	...
	{
		if (command.startsWith("select_m") || command.startsWith("voiced_select_m")) {
			if (!Config.ACTIVE_MISSION) {
				activeChar.sendMessage("[WARN]:The Mission is Temporarily unavailable.");
				return false;
			} 
			try {
				String type = (target != null) ? target.trim() : "";
				if (type.isEmpty() && command.contains(" ")) {
					type = command.substring(command.indexOf(' ') + 1).trim();
				}
				if (type.isEmpty() && command.startsWith("voiced_select_m ")) {
					type = command.substring("voiced_select_m ".length()).trim();
				}
				if (type.isEmpty() && command.startsWith("select_m ")) {
					type = command.substring("select_m ".length()).trim();
				}
				if (type.isEmpty()) {
