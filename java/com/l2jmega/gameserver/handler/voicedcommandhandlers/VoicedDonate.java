...
public class VoicedDonate implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		...
		{
				// For voiced bypass, arguments are passed in `target`, not in `command`.
				final String argLine = (target != null && !target.trim().isEmpty()) ? target.trim() : "";
				StringTokenizer st1 = new StringTokenizer(argLine);
				if (!st1.hasMoreTokens())
				{
					activeChar.sendMessage("Invalid augment request.");
					return true;
				}
