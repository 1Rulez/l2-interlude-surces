...
public class VoicedAutoPotion implements IVoicedCommandHandler {
    ...
    public boolean useVoicedCommand(String command, Player activeChar, String params) {
        ...
        if (command.equals("acpon"))
        {
            ...
            {
                activeChar.sendMessage("Auto Potion is not available for phantoms.");
                return false;
            }
			...
			{
				activeChar.sendMessage("it is not allowed to use dead Auto Potion.");
				return false ;
			}
			if(activeChar.isInOlympiadMode())
			{
				activeChar.sendMessage("It is not allowed to use Auto Potion in the Olympics.");
				return false ;
			}
...
//                               Checking our health
                                if ((activeChar.getStatus().getCurrentHp() / activeChar.getMaxHp()) * 100 < ACP_HP_LVL && ACP_HP)
                                {
                                   ...
                                   if(activeChar.isInOlympiadMode())
                                   {
                                	   activeChar.sendMessage("No Auto Potion Olympics allowed.");
                                	   return ;
                                   }
                                    if (handlerHP != null)
                                    {
