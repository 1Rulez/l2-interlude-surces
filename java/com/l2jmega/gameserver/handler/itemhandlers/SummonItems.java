...
public class SummonItems implements IItemHandler
{
	...
	public void useItem(Playable playable, ItemInstance item, boolean forceUse)
	{
		...
		final NpcTemplate npcTemplate = NpcTable.getInstance().getTemplate(sitem.getId());
		if (npcTemplate == null)
			return;
		...
		if (npcTemplate.getType().equalsIgnoreCase("Agathion"))
		{
			if (activeChar.getAgathion() != null)
			{
				...
				item.setAgathion(activeChar, false);
				activeChar.sendMessage("Agathion has been unsummoned");
				return;
			}
			item.setAgathion(activeChar, true);
			item.checkProtected(activeChar);
			
			Broadcast.toSelfAndKnownPlayers(activeChar, new MagicSkillUse(activeChar, SkillTable.getInstance().getInfo(Config.SKILL_ID_AGATHION, 1).getId(), 1, 5000, 0));
			return;
		}
