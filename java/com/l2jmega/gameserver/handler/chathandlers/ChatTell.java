...
public class ChatTell implements IChatHandler
{
	...
	public void handleChat(int type, Player activeChar, String target, String text)
	{
		...
		// Fake players may not be in PlayerNameTable -> fallback to scan all players by name.
		if (receiver == null)
		{
			for (Player p : World.getInstance().getPlayers())
			{
				if (p != null && p.getName() != null && p.getName().equalsIgnoreCase(target))
				{
					receiver = p;
					break;
				}
		...
		if (receiver.isPhantom())
		{
			...
			if (receiver instanceof FakePlayer)
				FakePlayerUtilsAI.answerPlayers(activeChar, (FakePlayer) receiver, text);
		}
