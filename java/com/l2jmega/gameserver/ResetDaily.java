	...
	public void StartReset()
	{
		try
		{
			...
			for (String timeOfDay : Config.RESET_DAILY_TIME)
			{
				...
				{
					_log.warning("Reset_Daily: invalid time format '" + timeOfDay + "'. Expected HH:mm.");
					continue;
				}
				...
				try
				{
					hour = Integer.parseInt(splitTimeOfDay[0].trim());
					minute = Integer.parseInt(splitTimeOfDay[1].trim());
				}
				catch (NumberFormatException e)
				{
					_log.warning("Reset_Daily: invalid numeric time '" + timeOfDay + "'. Expected HH:mm.");
					continue;
				}
				
				if (hour < 0 || hour > 23 || minute < 0 || minute > 59)
				{
					_log.warning("Reset_Daily: out-of-range time '" + timeOfDay + "'. Expected HH:mm.");
					continue;
				}
			...
			{
				_log.warning("Reset_Daily: no valid time found in ResetDailyTime config. Using fallback 00:00.");
				
				testStartTime = Calendar.getInstance();
				testStartTime.set(Calendar.HOUR_OF_DAY, 0);
				testStartTime.set(Calendar.MINUTE, 0);
				...
				flush2 = testStartTime.getTimeInMillis() - currentTime.getTimeInMillis();
				NEXT = testStartTime;
			}
