...
public class SiegeGuardAI extends AttackableAI
{
	...
	protected void thinkAttack()
	{
		...
		if (_attackTimeout < System.currentTimeMillis() || attackTarget.isAlikeDead() || MathUtil.calculateDistance(actor, attackTarget, true) > 2000)
		{
		...
		if (willCastASpell())
		{
			...
			if (!defaultList.isEmpty())
			{
				...
				for (Creature cha : actor.getKnownTypeInRadius(Creature.class, 1000))
				{
					...
					{
					    // Validate castle & siege first
					    if (actor.getCastle() == null || actor.getCastle().getSiege() == null)
					        continue;
					    ...
					    if (cha instanceof Player)
					    {
					        ...
					        if (player.getClan() != null &&
					            actor.getCastle().getSiege().checkSides(player.getClan(), SiegeSide.DEFENDER, SiegeSide.OWNER))
					        {
					            isValid = true;
					        }
					    }
					    else if (cha instanceof Npc)
					    {
					        if (ArraysUtil.contains(clans, ((Npc) cha).getTemplate().getClans()))
					            isValid = true;
					    }
	...
	protected Creature targetReconsider(int range, boolean rangeCheck)
	{
		...
		if (!actor.getAggroList().isEmpty())
		{
			...
			final int aggroMostHated = (previousMostHated != null) ? actor.getHating(previousMostHated) : 0;
