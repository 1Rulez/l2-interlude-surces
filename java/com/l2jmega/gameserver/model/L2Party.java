...
{
	private static final int DISBANDED_FAKE_RETURN_TO_GIRAN_X = 83597;
	private static final int DISBANDED_FAKE_RETURN_TO_GIRAN_Y = 147888;
	private static final int DISBANDED_FAKE_RETURN_TO_GIRAN_Z = -3405;
	private static final long DISBANDED_FAKE_RETURN_DELAY_MIN_MS = 30000L;
	private static final long DISBANDED_FAKE_RETURN_DELAY_MAX_MS = 60000L;
	...
	public void addPartyMember(Player player)
	{
		...
		if (player instanceof FakePlayer)
			applyCurrentPartyModeToFake((FakePlayer) player);
	...
	private void applyCurrentPartyModeToFake(FakePlayer fake)
	{
		PartyMode mode = PartyMode.FOLLOW;
		for (Player member : _members)
		{
			if (member instanceof FakePlayer)
			{
				mode = ((FakePlayer) member).getPartyMode();
				break;
			}
		}
	...
	public void removePartyMember(Player player, MessageType type)
	{
		...
		if (player instanceof FakePlayer)
		{
			...
			final boolean keepTournamentState = fake.isTour() || fake.isArenaProtection() || fake.isInArenaEvent() || fake.isArenaAttack();

			if (!keepTournamentState && _disbanding)
				scheduleDisbandedFakeReturnToGiran(fake);
			else if (!keepTournamentState)
			{
				Location loc = MapRegionTable.getInstance().getLocationToTeleport(fake, TeleportType.TOWN);
				fake.teleToLocation(loc.getX(), loc.getY(), loc.getZ(), 20);
		...
		// If only phantoms remain, disband party automatically.
		if (!_disbanding && !_members.isEmpty())
		{
			boolean hasRealPlayer = false;
			for (Player m : _members)
			{
				if (m != null && !m.isPhantom())
				{
					hasRealPlayer = true;
					break;
				}
			...
			{
				_disbanding = true;
				// Use a copy to avoid concurrent modification during removePartyMember recursion.
				final List<Player> snapshot = new ArrayList<>(_members);
				for (Player member : snapshot)
				{
					if (member != null)
					{
						member.sendPacket(SystemMessageId.PARTY_DISPERSED);
	...
	private static void scheduleDisbandedFakeReturnToGiran(FakePlayer fake)
	{
		if (fake == null)
			return;
		
		if (fake.isTour() || fake.isArenaProtection() || fake.isInArenaEvent() || fake.isArenaAttack())
			return;
		...
		final long delay = Rnd.get((int) DISBANDED_FAKE_RETURN_DELAY_MIN_MS, (int) DISBANDED_FAKE_RETURN_DELAY_MAX_MS);
		ThreadPool.schedule(() ->
		{
			if (fake.isOnline())
			{
				fake.teleToLocation(DISBANDED_FAKE_RETURN_TO_GIRAN_X, DISBANDED_FAKE_RETURN_TO_GIRAN_Y, DISBANDED_FAKE_RETURN_TO_GIRAN_Z, 20);
				FakeHelpers.giveBuffsByClass(fake);
				fake.heal();
				fake.assignDefaultAI();
			}
	...
	{
		final List<Player> membersForCalc;
		final int topLvlForCalc;
		if (FakePlayerConfig.FAKE_PARTY_IGNORE_XP_PENALTY)
		{
			// Phantoms should not affect party EXP/SP distribution (no level-diff penalty, no share).
			final List<Player> rewardedRealMembers = new ArrayList<>(rewardedMembers.size());
			for (Player member : rewardedMembers)
			{
				if (member != null && !member.isPhantom())
					rewardedRealMembers.add(member);
			}
			
			int topLvlReal = 0;
			for (Player member : rewardedRealMembers)
			{
				if (member.getLevel() > topLvlReal)
					topLvlReal = member.getLevel();
			}
			membersForCalc = rewardedRealMembers;
			topLvlForCalc = (topLvlReal > 0) ? topLvlReal : topLvl;
		}
		else
		{
			membersForCalc = rewardedMembers;
			topLvlForCalc = topLvl;
		}
		
		final List<Player> validMembers = getValidMembers(membersForCalc, topLvlForCalc);
		...
		for (Player member : rewardedMembers)
		{
			...
			if ((!FakePlayerConfig.FAKE_PARTY_IGNORE_XP_PENALTY || !member.isPhantom()) && validMembers.contains(member))
			{
