...
public abstract class AbstractOlympiadGame
{
	...
	protected final void addPointsToParticipant(Participant par, int points)
	{
		...
		{
			updateParticipantResult(par, true);
			return;
		}
	...
	protected final void removePointsFromParticipant(Participant par, int points)
	{
		if (Config.ALT_OLY_UNLIMITED_TEST_MODE)
		{
			updateParticipantResult(par, false);
			return;
		}
	...
	private static void updateParticipantResult(Participant par, boolean won)
	{
		if (par == null)
			return;
		
		final String selectSql = won ? OLYMPIAD_POINTS_WIN : OLYMPIAD_POINTS_LOST;
		final String updateSql = won ? OLYMPIAD_UPDATE_WIN : OLYMPIAD_UPDATE_LOST;
		
		int currentPoints = 0;
		int competitionsDone = 0;
		int competitionsResult = 0;
		...
		try
		{
			...
			while (rset.next())
			{
				...
				competitionsDone = rset.getInt("competitions_done");
				competitionsResult = rset.getInt(won ? "competitions_won" : "competitions_lost");
			}
			rset.close();
			statement.close();
		}
		...
		{
			_log.warning("Olympiad test mode: failed to load participant result data: " + e);
		}
		finally
		{
			CloseUtil.close(con);
		...
		try (Connection updateCon = L2DatabaseFactory.getInstance().getConnection();
			 PreparedStatement statement = updateCon.prepareStatement(updateSql))
		{
			statement.setInt(1, currentPoints);
			statement.setInt(2, competitionsDone + 1);
			statement.setInt(3, competitionsResult + 1);
			statement.setInt(4, par.objectId);
			statement.execute();
		}
		catch (Exception e)
		{
			_log.warning("Olympiad test mode: failed to update participant result data: " + e);
		}
	}
	...
	protected static SystemMessage checkDefaulted(Player player)
	{
		...
		if (player.getClient().isDetached() && !(player instanceof FakePlayer))
			return SystemMessage.getSystemMessage(SystemMessageId.THE_GAME_HAS_BEEN_CANCELLED_BECAUSE_THE_OTHER_PARTY_ENDS_THE_GAME);
	...
	protected static final void removals(Player player)
	{
		try
		{
			...
			if (player instanceof FakePlayer)
				FakeHelpers.ensureOlympiadLoadout((FakePlayer) player);
