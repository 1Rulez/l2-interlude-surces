...
public class Olympiad
{
	...
	private void activateUnlimitedTestMode()
	{
		cancelScheduledTask(_scheduledCompStart);
		cancelScheduledTask(_scheduledCompEnd);
		cancelScheduledTask(_scheduledOlympiadEnd);
		cancelScheduledTask(_scheduledWeeklyTask);
		cancelScheduledTask(_scheduledValdationTask);
		
		_period = 0;
		_inCompPeriod = true;
		_compStarted = true;
		_compStart = Calendar.getInstance();
		_compEnd = Long.MAX_VALUE / 4;
		
		if (_gameManager == null || _gameManager.isCancelled() || _gameManager.isDone())
			_gameManager = ThreadPool.scheduleAtFixedRate(OlympiadGameManager.getInstance(), 1000, 30000);
		
		if (Config.ALT_OLY_ANNOUNCE_GAMES && (_gameAnnouncer == null || _gameAnnouncer.isCancelled() || _gameAnnouncer.isDone()))
			_gameAnnouncer = ThreadPool.scheduleAtFixedRate(new OlympiadAnnouncer(), 30000, 500);
		
		_log.info("Olympiad: Unlimited test mode enabled. Competition period is forced ON.");
	}
	...
	protected void sortHeroesToBe()
	{
		...
		// Collect class IDs already found from DB
		final java.util.Set<Integer> foundClasses = new java.util.HashSet<>();
		...
		// Supplement from in-memory nobles for players not found in DB
		// (e.g. fake players whose characters record was deleted on despawn)
		for (ClassId id : ClassId.VALUES)
		{
			if (id.level() != 3 || foundClasses.contains(id.getId()))
				continue;
			...
			{
				final StatsSet noble = entry.getValue();
				if (noble.getInteger(CLASS_ID) != id.getId())
					continue;
				
				final int compDone = noble.getInteger(COMP_DONE);
				final int compWon = noble.getInteger(COMP_WON);
				if (compDone < Config.ALT_OLY_MIN_MATCHES || compWon <= 0)
					continue;
				
				final int points = noble.getInteger(POINTS);
				if (points > bestPoints || (points == bestPoints && compDone > bestDone) || (points == bestPoints && compDone == bestDone && compWon > bestWon))
				{
					bestCharId = entry.getKey();
					bestName = noble.getString(CHAR_NAME);
					bestPoints = points;
					bestDone = compDone;
					bestWon = compWon;
				}
			...
			if (bestCharId > 0 && bestName != null)
			{
				StatsSet hero = new StatsSet();
				hero.set(CLASS_ID, id.getId());
				hero.set(CHAR_ID, bestCharId);
				hero.set(CHAR_NAME, bestName);
