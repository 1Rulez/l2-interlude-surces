...
public abstract class Inventory extends ItemContainer
{
	...
	public ItemInstance dropItem(String process, int objectId, int count, Player actor, WorldObject reference)
	{
	    ...
	    synchronized (item)
	    {
	        ...
	        boolean isTimed =
	            Config.ENABLE_TIMED_ITEMS &&
	            Config.LIST_TIMED_ITEMS.contains(item.getItemId());

	        // PARTIAL DROP (stack split)
	        if (item.getCount() > count)
	        {
	            ...
	            ItemInstance dropped = ItemTable.getInstance()
	                    .createItem(process, item.getItemId(), count, actor, reference);
	            dropped.updateDatabase();

	            // 🔥 TIMED ITEM FIX (REGISTER NEW OBJECT)
	            if (isTimed)
	            {
	                TimedItemManager.getInstance().registerItem(dropped);
	            }
	    ...
	    // FULL ITEM DROP
	    ItemInstance dropped = dropItem(process, item, actor, reference);

	    // 🔥 TIMED ITEM FIX (KEEP TIMER ON DROPPED ITEM)
	    if (dropped != null &&
	        Config.ENABLE_TIMED_ITEMS &&
	        Config.LIST_TIMED_ITEMS.contains(dropped.getItemId()))
	    {
	        TimedItemManager.getInstance().registerItem(dropped);
	    }
	...
	public ItemInstance[] equipItemAndRecord(ItemInstance item)
	{
		   Inventory.ChangeRecorder recorder = newRecorder();
		   if (item == null) {
			   _log.warning("[Inventory] Tried to equip null item in equipItemAndRecord.");
			   removePaperdollListener(recorder);
			   return recorder.getChangedItems();
		   }
		   try
	 ...
	 */
	   ...
	   {
		   if (item == null) {
			   _log.warning("[Inventory] Tried to equip null item in equipItem.");
			   return;
		   }
		   if (getOwner() instanceof Player)
		   {
	...
	{
		final java.util.List<OnEquipListener> listenersSnapshot = new java.util.ArrayList<>(_paperdollListeners);
		...
		for (ItemInstance element : _paperdoll)
		{
			...
			for (OnEquipListener listener : listenersSnapshot)
			{
				if (listener == null)
					continue;
