...
public class Clan
{
    ...
    private static final String ADD_OR_UPDATE_SKILL = "INSERT INTO clan_skills (clan_id,skill_id,skill_level) VALUES (?,?,?) ON DUPLICATE KEY UPDATE skill_level=?";
    ...
    private static final String ADD_OR_UPDATE_PRIVILEDGE = "INSERT INTO clan_privs (clan_id,rank,privs) VALUES (?,?,?) ON DUPLICATE KEY UPDATE privs=?";
	...
	public Clan(int clanId, int leaderId)
	{
		...
		{
			// Load members.
			ps = con.prepareStatement(LOAD_MEMBERS);
			ps.setInt(1, _clanId);
		...
		{
			LOGGER.warn("Error while loading clan privileges for clan " + _clanId + ": " + e.getMessage());
			// Continue without privileges if table doesn't exist
		}
		finally
		{
			try
			...
			{
				LOGGER.warn("Error closing resources in clan privileges loading: " + e.getMessage());
			}
		}
		...
		{
			LOGGER.warn("Error while loading clan skills for clan " + _clanId + ": " + e.getMessage());
		...
		finally
		{
			...
			{
				LOGGER.warn("Error closing resources in clan skills loading: " + e.getMessage());
			}
		}
		...
		finally
		{
			...
			{
				LOGGER.warn("Error closing resources in clan loading: " + e.getMessage());
			}
		}
	...
	/**
	 * Get the custom holy NPC for this clan
	 * @return the custom holy NPC or null if none
	 */
	public Npc getCustomHoly()
	{
		return _customHoly;
	}
	
	/**
	 * Set the custom holy NPC for this clan
	 * @param customHoly the custom holy NPC to set
	 */
	public void setCustomHoly(Npc customHoly)
	{
		_customHoly = customHoly;
	}
	...
	public Player[] getOnlineMembers()
	{
		...
		for (ClanMember temp : new ArrayList<>(_members.values()))
		{
	...
	private void setReputationScore(int value)
	{
		...
		// Optimized: Cache online members to avoid multiple calls
