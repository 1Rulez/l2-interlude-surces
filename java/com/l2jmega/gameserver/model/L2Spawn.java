...
public final class L2Spawn implements Runnable
{
	...
	public Npc doSpawn(boolean isSummonSpawn)
	{
	    try
	    {
	        ...
	        // 🔹 SE tivermos uma instância definida no L2Spawn, aplica aqui
	        if (_instance != null)
	        {
	            _npc.setInstance(_instance, false);
	        }

	        // 🔹 Agora sim inicializa e spawna no mundo já na instância correta
	        initializeAndSpawn();
	...
	private void initializeAndSpawn()
	{
		...
		if (Config.CHAMPION_FREQUENCY > 0)
		{
			...
			if (_npc instanceof Monster monster && !getTemplate().cantBeChampion() && _npc.getLevel() >= Config.CHAMP_MIN_LVL && _npc.getLevel() <= Config.CHAMP_MAX_LVL && !_npc.isRaid() && !monster.isRaidMinion() && !monster.isMinion())
				monster.setChampion(Rnd.get(100) < Config.CHAMPION_FREQUENCY);
		}
	...
	// fields
	private Base.Dungeon.Instance _instance = null;

	// novo setter
	public void setInstance(Base.Dungeon.Instance instance)
	{
	    _instance = instance;
	}
	public Base.Dungeon.Instance getInstanceObject()
	{
	    return _instance;
	}
