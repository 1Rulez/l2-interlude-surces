...
public class CursedWeapon
{
	...
	{
	    // Remove previous level first (important)
	    _player.removeSkill(_skillId);

	    // Add correct stage level
	...
	{
	    // Prevent multiple cursed weapon drops
	    if (_isDropped || _isActivated || _item != null)
	        return false;

	    synchronized (this)
	    {
	        // Double check inside lock (VERY IMPORTANT)
	        if (_isDropped || _isActivated || _item != null)
	            return false;
	        ...
	        if (Rnd.get(1000000) < _dropRate)
	        {
	            ...
	            _isDropped = true; // Lock state AFTER successful drop spawn

	            _endTime = System.currentTimeMillis() + _duration * 3600000L;

	            _overallTimerTask = ThreadPool.scheduleAtFixedRate(
	                    new OverallTimerTask(), 60000L, 60000L);

	            _dropTimerTask = ThreadPool.schedule(
	                    new DropTimerTask(), 3600000L);
	...
	public void activate(Player player, ItemInstance item)
	{
	    ...
	    int playerCW = player.getCursedWeaponEquippedId();
	    int newCW = item.getItemId();

	    // Cursed weapon validation
	    if (!CursedWeaponsManager.getInstance().isCursed(newCW))
	        return;

	 // HARD CW SLOT LOCK
	    if (playerCW != 0)
	    {
	        player.sendMessage("You cannot possess more than one Cursed Weapon.");

	        item.decayMe(); // remove from world first
	        player.destroyItem("CursedWeapon", item.getObjectId(), 1, null, false);
	    ...
	    if (weapon == null || weapon.getItemId() != _itemId)
	        return;

	    // NOW activate CW state
	    _isActivated = true;
	    ...
	    _numberBeforeNextStage = Rnd.get(
	            (int) Math.round(_stageKills * 0.5),
	            (int) Math.round(_stageKills * 1.5));

	    _hungryTime = _durationLost * 60;

	    _dailyTimerTask =
	            ThreadPool.scheduleAtFixedRate(new DailyTimerTask(), 60000L, 60000L);
	...
	public void loadData()
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement(
		             "SELECT * FROM cursed_weapons WHERE itemId=?"))
		{
	...
	private void removeFromDb()
	{
		...
		catch (SQLException e)
		{
			...
			_log.log(Level.SEVERE, "CursedWeapon: Failed to remove data.", e);
		}
	}
