...
public class Hero
{
	...
	private static final String GET_DIARIES ="SELECT * FROM heroes_diary WHERE char_id=? ORDER BY event_time ASC";
	private static final String UPDATE_DIARIES ="INSERT INTO heroes_diary (char_id, event_time, action, param) values(?,?,?,?)";
	...
	public void loadDiary(int charId)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			while (rset.next())
			{
				...
				long time = rset.getLong("event_time");
	...
	public synchronized void computeNewHeroes(List<StatsSet> newHeroes)
	{
		...
		// Auto-activate hero status for fake players since they can't interact with NPC
		for (StatsSet hero : newHeroes)
		{
			String name = hero.getString(Olympiad.CHAR_NAME);
			Player player = World.getInstance().getPlayer(name);
