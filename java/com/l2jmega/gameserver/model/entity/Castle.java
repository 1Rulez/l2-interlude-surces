...
public class Castle
{
	...
	private static final String LOAD_SIEGE_GUARDS = "SELECT npcId, x, y, z, heading, respawnDelay FROM castle_siege_guards WHERE castleId=?";
	...
	private final List<L2Spawn> _siegeGuardSpawns = new ArrayList<>();
	...
	public void loadSiegeGuards()
    {
        _siegeGuardSpawns.clear();
             ...
             PreparedStatement ps = con.prepareStatement(LOAD_SIEGE_GUARDS))
        {
            ...
            while (rs.next())
            {
                ...
                if (template == null)
                {
                    _log.warning("Castle " + getName() + " siege guard npcId " + npcId + " template missing.");
                    continue;
                }
                ...
                _siegeGuardSpawns.add(spawn);
            }
            rs.close();
        }
        catch (Exception e)
        {
            _log.warning("Could not load siege guards for castle " + getName() + ": " + e.getMessage());
        }
    }
	...
	public void setOwner(Clan clan)
	{
	    ...
	    // existing owner assignment code here...

	    // 🔒 RESET reward only when owner changes
	    if (clan != null && getSiege() != null)
	    {
	        getSiege().resetCastleReward();
	    }
	...
	{
		if (!Config.ENABLE_SIEGE_GUARDS_SPAWN)
			return;
		
		// Load guards only when needed (prevents NpcTable not loaded errors)
        if (_siegeGuardSpawns.isEmpty())
            loadSiegeGuards();
		...
		// Neutral Siege Guards
        for (L2Spawn spawn : _siegeGuardSpawns)
        {
            try
            {
            ...
            {
                _log.warning("Could not spawn siege guard: " + e.getMessage());
            }
        }
    }
