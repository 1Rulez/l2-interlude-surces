...
import com.l2jmega.gameserver.communitybbs.AnnouncementsUpdateManager;
...
import com.l2jmega.hopzone.eu.VDSystemManager;
...
import Base.AutoBackup.BackupDBSave;
import Base.BonusXp.RunesSystemManager;
...
import Base.Event.RaceOfWar.RaceOfWarScheduler;
import Base.LimitedTimeItems.TimedItemHtmlTask;
import Base.LimitedTimeItems.TimedItemTask;
import Base.MultisellAnnunces.AnnouceMultiSell;
import Base.RandomFightEvent.RandomFightScheduler;
import Base.achievementEngine.AchievementsManager;
import Base.announcerTopPlayer.AnnounceOnlinePlayers;
...
public class GameServer
{
	...
	public GameServer() throws Exception
	{
		...
		StringUtil.printSection("Developed Active Project 2025");
		L2jMega.info();
		StringUtil.printSection("Private Files by Dev-Fol / L2ELYSIAN REAMLS.");
		...
		if (Config.RESET_DAILY_ENABLED)
			...
			LOGGER.info("Reset Daily: disabled.");
		...
		StringUtil.printSection("Fake Players");
		...
		StringUtil.printSection("DressMe Manager L2Elysian");
		{
	    DressMeData.getInstance();
		}
	    
	    StringUtil.printSection("CapsuleBox L2Elysian");
	    {
	    CapsuleBoxData.getInstance();
	    }
	    
	    StringUtil.printSection("Achievments System L2Elysian");
	    {
	    AchievementsManager.getInstance();
	    }
	    
		StringUtil.printSection("Dolls Data L2Elysian");
		{
		DollsData.getInstance();
		}

		StringUtil.printSection("DataBase Auto Save L2Elysian");
		{
		BackupDBSave.init();  // respects enableBackup from properties
		}
		
		StringUtil.printSection("ANNOUNCE ONLINE PLAYERS L2Elysian");
		if (Config.ALLOW_ANNOUNCE_ONLINE_PLAYERS)
		{
			AnnounceOnlinePlayers.getInstance();
		}
		
		StringUtil.printSection("DunGeon Event System L2Elysian");
		{
		DungeonManager.getInstance();
		}
		
		StringUtil.printSection("HeroBoss Event System L2Elysian");
		{
		    BossEvent.getInstance();
		    NextBossEvent.getInstance().startCalculationOfNextEventTime();  // <-- add this
		}
		
		StringUtil.printSection("AuCtion System L2Elysian");
		
		StringUtil.printSection("AnnouceSell Donate System L2Elysian");
		{
		    AnnouceMultiSell.getInstance();
		}
		
		StringUtil.printSection("Random Fight 1-X-1 system L2Elysian");
		if (Config.ALLOW_RANDOM_FIGHT)
		{
		    RandomFightScheduler.init();
		}
	    
	    StringUtil.printSection("Riberth Engine L2Elysian");

	    
	    StringUtil.printSection("RaceOFWar Event L2Elysian");
	    if (Config.ENABLE_RACE_OF_WAR)
	    {
	        RaceOfWarScheduler.init();
	    }

	    
	    StringUtil.printSection("Elysian God Zone System. L2Elysian");
	    {
	    	_log.info("Ultimate Zone loaded successfully.");
	    }
	    
	    Util.printSection("Bonus Xp System. L2Elysian");
	    {
	    RunesSystemManager.getInstance();
	    }
	    
	 // Load config & other systems
	    Util.printSection("Start Timed Items Task. L2Elysian");
	    if (Config.ENABLE_TIMED_ITEMS)
	    {
	        TimedItemHtmlTask.startTask();
	        TimedItemTask.startTask();
	    }

	    if (Config.ENABLE_REWARD_DAILY)
	    {
	        Util.printSection("Start Daily Reward Task L2Elysian");
	        ...
	        ThreadPool.scheduleAtFixedRate(() ->
	        {
	            for (Player player : World.getInstance().getPlayers())
	            {
	                ...
	                {
	                    DailyRewardManager.getInstance().onPlayerTick(player);
	                }
	            }
	        }, 60000, 60000);
	    }
     ...
     // Load config & other systems
        Util.printSection("Start DailyReward Task. L2Elysian");
        if (Config.ENABLE_REWARD_DAILY)
        {
            System.out.println("Daily Reward System: Enabled");
            DailyRewardManager.getInstance();
        }
        else
        {
            System.out.println("Daily Reward System: Disabled");
        }
        
	    StringUtil.printSection("Global Vote system L2Elysian");
