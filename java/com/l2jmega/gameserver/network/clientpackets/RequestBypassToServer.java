...
import phantom.helpers.FakePartyTeleportSupport;
...
public final class RequestBypassToServer extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (!FloodProtectors.performAction(getClient(), Action.SERVER_BYPASS)
			&& !_command.startsWith("voiced_getbuff")
			&& !_command.startsWith("_bbsstats")
			&& !_command.startsWith("_bbsOlStats"))
			return;
		...
		if (_command.isEmpty())
		{
			_log.warning(activeChar.getName() + " sent an empty requestBypass packet (possible shortcut glitch).");
			if (!activeChar.isGM())
				activeChar.logout();
			return;
		}
		...
		try
		{
			...
			{
				String fullCmd = _command.substring(7);
				String voicedCmd = fullCmd;
				String voicedParams = null;
				int spIdx = fullCmd.indexOf(' ');
				if (spIdx != -1)
				{
					voicedCmd = fullCmd.substring(0, spIdx);
					voicedParams = fullCmd.substring(spIdx + 1).trim();
				}
				IVoicedCommandHandler ach = VoicedCommandHandler.getInstance().getHandler(voicedCmd);
				if (ach == null)
				{
					activeChar.sendMessage("The command " + voicedCmd + " does not exist!");
					_log.warning("No handler registered for command '" + _command + "'");
					return;
				}
				ach.useVoicedCommand(voicedCmd, activeChar, voicedParams);
			}
			...
			else if (_command.startsWith("tele_tournament"))
			{
				...
				for (Gatekeeper knownChar : activeChar.getKnownTypeInRadius(Gatekeeper.class, 300))
				{
					if (knownChar != null)
					{
						...
						activeChar.teleToLocation(targetX, targetY, Config.Tournament_locz, 0);
						FakePartyTeleportSupport.syncFakePartyToCoordinates(activeChar, targetX, targetY, Config.Tournament_locz, 0);
			...
			if (_command.startsWith("bp_reward"))
			{
			    ...
			    {                         	  
					...
					activeChar.setPlayerInstance(null);						
			    ...
			    {
			    	 activeChar.sendMessage("You cannot receive the prize");  
			    }
