...
public final class Say2 extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
	    ...
	    if(_text.equalsIgnoreCase(".regrandom") || _text.equalsIgnoreCase(".unrandom"))
	    return;
	...
	private static void checkRandomFight(String text, Player player)
	{
	    if (text.equalsIgnoreCase(".regrandom"))
	    {
	        if (!Base.RandomFightEvent.RandomFight.isRegistering())
	        {
	            player.sendMessage("Random Fight is not open for registration.");
	            return;
	        }

	        if (Base.RandomFightEvent.RandomFight.isRegistered(player))
	        {
	            player.sendMessage("You already registered.");
	            return;
	        }

	        Base.RandomFightEvent.RandomFight.register(player);
	        player.sendMessage("You registered to the event!");
	        return;
	    }

	    if (text.equalsIgnoreCase(".unrandom"))
	    {
	        if (!Base.RandomFightEvent.RandomFight.isRegistering())
	        {
	            player.sendMessage("Random Fight is not open for registration.");
	        ...
	        Base.RandomFightEvent.RandomFight.unregister(player);
