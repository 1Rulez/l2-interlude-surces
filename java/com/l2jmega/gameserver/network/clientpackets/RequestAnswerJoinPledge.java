...
public final class RequestAnswerJoinPledge extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (activeChar instanceof phantom.FakePlayer) {
			if (phantom.FakePlayerConfig.FAKE_ALLOW_CLAN_INVITE) {
				autoAccept = (com.l2jmega.commons.random.Rnd.get(100) < phantom.FakePlayerConfig.FAKE_CLAN_INVITE_ACCEPT_CHANCE);
			}
		}

		// Decline: client sends _answer == 0. Accept: _answer == 1. Fakes may auto-accept despite _answer == 0.
		// Do NOT treat _answer != 0 as decline — that broke real players accepting (they got "cancelled" messages).
		if (_answer == 0 && !autoAccept) {
