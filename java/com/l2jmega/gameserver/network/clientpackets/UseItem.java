...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
        ...
        if (getClient().getActiveChar().isInsideZone(ZoneId.ELYSIAN_ULTIMATE) && !L2ElysianUltimateZone.checkItem(item))
       {
		...
		// Disable all heal/CP potions for phantoms on all events
		if (activeChar.isPhantom() && (item.isPotion() || item.isElixir()))
		{
			boolean inEvent = (activeChar._inEventTvT && TvT.is_started()) || (activeChar._inEventCTF && CTF.is_started())
				|| activeChar.isArenaProtection() || activeChar.isInArenaEvent() || activeChar.isInsideZone(ZoneId.TOURNAMENT)
				|| AnonymousPvPEvent.isPlayerInEvent(activeChar);
			if (activeChar instanceof FakePlayer)
			{
				FakePlayer fp = (FakePlayer) activeChar;
				inEvent = inEvent || fp.isFakePvp() || fp.isFakeFarm() || fp.isFakeKTBEvent() || fp.isFakeEvent() || fp.isTour();
			}
			if (inEvent)
			{
				return;
