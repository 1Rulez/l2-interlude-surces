...
public final class RequestJoinParty extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (target == null)
		{
			// Fallback: search by name among online players (FakePlayers are not in PlayerNameTable/DB)
			target = World.getInstance().getPlayers().stream()
				.filter(p -> p != null && _name.equalsIgnoreCase(p.getName()))
		...
		if (!(target instanceof FakePlayer) && target.getClient() != null && target.getClient().isDetached())
		{
		...
		{
			target.sendMessage("Unable to do any action while PIN is not submitted");
			return;
		}
		...
		{
			if (target.isSitting() || target.isInStoreMode())
			{
				requestor.sendMessage(target.getName() + " is busy (sitting or trading).");
				return;
			}
			handleFakePlayerInvite(target, requestor);
			return;
		}
	...
	private static void handleFakePlayerInvite(Player target, Player requestor)
	{
		if (!requestor.isInParty())
			requestor.setParty(new L2Party(requestor, _itemDistribution));
		...
		int delay = Rnd.get(2000, 3000);
		ThreadPool.schedule(() ->
		{
			try
			{
				L2Party p = requestor.getParty();
				if (p == null || !p.isLeader(requestor))
					return;
				
				if (p.getMemberCount() >= 9)
					return;
				...
				if (target instanceof FakePlayer)
				{
					FakePlayer fake = (FakePlayer) target;
					fake.setFakeAi(new PartyFollowerAI(fake));
					fake.setPartyMode(PartyMode.FOLLOW);
				}
				requestor.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.S1_JOINED_PARTY).addCharName(target));
			}
			catch (Exception e)
			{
				// ignore
