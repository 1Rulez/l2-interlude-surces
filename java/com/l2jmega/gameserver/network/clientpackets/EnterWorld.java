...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		ArenaTask.cleanupStaleTournamentState(activeChar);
		
		// Check if player was inside Ultimate Zone but event is inactive
		if (activeChar.isInsideZone(ZoneId.ELYSIAN_ULTIMATE) && !L2ElysianUltimateZone.EVENT_ACTIVE) {
		    activeChar.setInsideZone(ZoneId.ELYSIAN_ULTIMATE, false);
		    ...
		    activeChar.sendPacket(new ExShowScreenMessage(
		        "You were removed from the Ultimate Zone because the event is not active.",
		        5000,
		        ExShowScreenMessage.SMPOS.TOP_CENTER,
		        true
		    ));

		    _log.info("[UltimateZone]: Player " + activeChar.getName() + " was outside the event and kicked from the zone.");
		}
		
		activeChar.setLastIp(getClient().getConnection().getInetAddress().getHostAddress());
		...
		DailyRewardManager.getInstance().onLogin(activeChar);
		...
		// ✅ Load timed items ONCE (logout + restart safe)
		TimedItemManager.getInstance().loadTimedItemsForPlayer(activeChar);
		TimedItemManager.getInstance().registerExistingInventory(activeChar);

		// ✅ New character logic (run once)
		...
		// Show PIN panel only if PIN feature is active for this character
		// and there is a stored PIN (pin != 0). If PIN was removed in DB (pin = 0),
		// this screen will not be shown.
		if(!activeChar.getPincheck() && activeChar.getPin() != 0)
		{
			   StringBuilder sb = new StringBuilder();
		       NpcHtmlMessage html = new NpcHtmlMessage(1);
		               
		       sb.append("<html><head><title>Character Pin Panel</title></head>");
		       sb.append("<body>");      
		       sb.append("<center>");
		       ...
		       sb.append("<br1><font color=\"00FF00\">"+activeChar.getName()+"</font>, use this interface to enable pin secirity.</td></tr></table></center>");
		       sb.append("<center>");
		       ...
		       sb.append("<td valign=\"top\">Please enter your PIN:<edit var=\"dapin\" width=80 height=15>");
		       sb.append("<br1>info or something (can delete)</td>");
		       sb.append("</tr>");
		       sb.append("</table>");
		       sb.append("<br>");
		       sb.append("<center>");
		       sb.append("<button value=\"Submit\" action=\"bypass -h enterpin $dapin\" width=75 height=21 back=\"L2UI_ch3.Btn1_normalOn\" fore=\"L2UI_ch3.Btn1_normal\">");
		       sb.append("</center>");
		       sb.append("<center>");
		...
		if ((!activeChar.isGM() && (!activeChar.isInSiege() || activeChar.getSiegeState() <= 0) && activeChar.isInsideZone(ZoneId.SIEGE)) || (!activeChar.isInsideZone(ZoneId.PEACE) && Olympiad.getInstance().playerInStadia(activeChar)))
		{
		...
		// Means that it's not ok multiBox situation, so logout (GM bypass)
		if (!activeChar.isGM() && !activeChar.checkMultiBox())
		{
		...
		if (Config.MULTIBOX_PROTECTION_ENABLED && !activeChar.isGM())
	...
	public static void Announce(Player activeChar)
	{
		if (Config.ANNOUNCE_HERO_ON_ENTER && activeChar.isHero())
		{
			...
			if (exists)
			{
				...
				Announcement.Announce("The Hero " + activeChar.getName() + " with class " + className + " is online!");
			}
		...
		if (clan != null)
		{
			...
			if (clanHall != null)
			{
				if ((activeChar.getObjectId() == activeChar.getClan().getLeaderId() && Config.ANNOUNCE_HALL_OWNERS))
					Announcement.Announce(activeChar.getName() + ", Owner " + clanHall.getName() + " of " + clanHall.getLocation() + " is online!");
			}
		}
			...
			if (activeChar.getClan() != null)
			{
				if (activeChar.getClan().getLeaderName().equals(activeChar.getName()))
				{
					if (CastleManager.getInstance().getCastleByOwner(activeChar.getClan()) != null)
						Announcement.Announce("The Lord " + activeChar.getName() + " owner of the Castle " + CastleManager.getInstance().getCastleByOwner(activeChar.getClan()).getName() + " is online!");
				}
			}
		}
