...
{
	private static final String SHORTCUT_REGISTER_PACKET = "com.l2jmega.gameserver.network.serverpackets.ShortCutRegister";
	...
	protected void runImpl()
	{
		...
		switch (_skillType)
		{
			case 0: // Elysian Rebirth skills.
				...
				if (_skillId> 0)
				{
					if (!activeChar.destroyItemByItemId("SkillLearn", _skillId, 1, trainer, false))
					{
		...
		if (_skillLevel > 1)
		{
			for (L2ShortCut sc : player.getAllShortCuts())
			{
				if (sc.getId() == _skillId && sc.getType() == L2ShortCut.TYPE_SKILL)
				{
					...
					sendPacketReflective(SHORTCUT_REGISTER_PACKET, new Class<?>[]
					{
						L2ShortCut.class
					}, newsc);
