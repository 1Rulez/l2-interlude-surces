...
{
	private static final int MAX_HTML_LENGTH = 16384;
	...
	{
		if (text != null && text.length() > MAX_HTML_LENGTH)
		{
			_html = "<html><body>Html was too long.</body></html>";
			LOGGER.warn("NpcHtmlMessage: html is too long ({} > {}). File: {}", text.length(), MAX_HTML_LENGTH, _file);
			return;
		}
		_html = text;
	}
