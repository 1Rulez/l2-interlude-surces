...
import Base.custom.event.AnonymousPvPEvent.FakeIdentity;
...
public class CharInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		/*
		 * Identity Rendering Priority:
		 * 1. Anonymous PvP Identity
		 * 2. Zone Name System
		 * 3. Normal Player Name
		 */

		boolean anonymousEvent = AnonymousPvPEvent.isPlayerInEvent(_activeChar);
		FakeIdentity identity = anonymousEvent ? AnonymousPvPEvent.getFakeIdentity(_activeChar) : null;

	    if (anonymousEvent && identity != null)
	    {
	    writeS(identity.getFakeName());
	    }
	    else if (Config.ALLOW_ZONE_NAME && _activeChar.isNewNameZone())
	    {
		...
		if (anonymousEvent)
		{
		    writeD(AnonymousPvPEvent.getAnonymousRaceId(_activeChar));
		}
		else
		{
		...
		writeD(_activeChar.getAppearanceClassId());
		...
		writeD(dress == null ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_GLOVES)
		        : (dress.getGlovesId() == 0 ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_GLOVES) : dress.getGlovesId()));

		writeD(dress == null ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_CHEST)
		        : (dress.getChestId() == 0 ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_CHEST) : dress.getChestId()));

		writeD(dress == null ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_LEGS)
		        : (dress.getLegsId() == 0 ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_LEGS) : dress.getLegsId()));

		writeD(dress == null ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_FEET)
		        : (dress.getFeetId() == 0 ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_FEET) : dress.getFeetId()));

		writeD(_inv.getPaperdollItemId(Inventory.PAPERDOLL_BACK));
		writeD(_inv.getPaperdollItemId(Inventory.PAPERDOLL_RHAND));

		writeD(dress == null ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_HAIR)
		        : (dress.getHairId() == 0 ? _inv.getPaperdollItemId(Inventory.PAPERDOLL_HAIR) : dress.getHairId()));
		...
		{
			writeF(_activeChar.getClientCollisionRadius());
			writeF(_activeChar.getClientCollisionHeight());
		}
		...
		/*
		 * TITLE RENDERING PRIORITY:
		 * 1. Anonymous PvP Fake Title (with kill counter)
		 * 2. TvT / CTF = Empty Title
		 * 3. AIO Title
		 * 4. Normal Player Title
		 */

		if (anonymousEvent && identity != null)
		{
		    writeS(identity.getFakeTitle());
		}
		else if ((TvT.is_started() || TvT.is_teleport()) && _activeChar._inEventTvT)
		{
		    writeS("");
		}
		else if ((CTF.is_started() || CTF.is_teleport()) && _activeChar._inEventCTF)
		{
		    writeS("");
		}
		else if (AioManager.getInstance().hasAioPrivileges(_activeChar.getObjectId()) 
		        || _activeChar.isAio())
		{
		...
		if ((TvT.is_started() || TvT.is_teleport()) 
		        || (CTF.is_started() || CTF.is_teleport()) 
		        || AnonymousPvPEvent.isPlayerInEvent(_activeChar))
		{
		    writeD(0);
		    writeD(0);
		    writeD(0);
		...
		if (_activeChar.isHero() && (((TvT.is_started() || TvT.is_teleport()) && _activeChar._inEventTvT) || ((CTF.is_started() || CTF.is_teleport()) && _activeChar._inEventCTF)))
			...
			if (AnonymousPvPEvent.isPlayerInEvent(_activeChar))
			{
			    writeC(0);
			}
		else
